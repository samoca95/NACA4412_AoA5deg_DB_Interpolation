function [top,bottom] = read_data_profiles(path,x_pts,y_pts,alphal,alphau,long,xc)
%UNTITLED5 Summary of this function goes here
%   Detailed explanation goes here

npoints=length(x_pts);

disp('No Rotation Read INT_FLD')
...
%     Ralphapp=eye(3);    
    if j<=np        
        Ralphap=[cos(top(j).alpha) sin(top(j).alpha) 0; ...
            ...
            0 0 1];
        for i=1:ln
            prod=[dUdx(i,j) dUdy(i,j) dUdz(i,j);
...
for j=1:2*np-1
    if j<=np
        Ralphap=[cos(top(j).alpha) sin(top(j).alpha) 0; ...
            ...
            0 0 1];
        for i=1:ln
            prod=[-2*(uu(i,j).*dUdx(i,j)+uv(i,j).*dUdy(i,j)) ...
...
for j=1:2*np-1
    if j<=np
        Ralphap=[cos(top(j).alpha) sin(top(j).alpha) 0; ...
            ...
            0 0 1];
        for i=1:ln
            prod=Ralphap*[F12(i,j); F13(i,j); F14(i,j)];
            top(j).pu(i,1)=prod(1);
            top(j).pv(i,1)=prod(2);
            top(j).pw(i,1)=prod(3);
        end
