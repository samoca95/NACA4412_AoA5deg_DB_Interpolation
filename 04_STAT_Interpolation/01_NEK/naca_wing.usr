...
!     Necessary to write averaged 2D fields in Nek
      ...
      real stat_temp3(lx1*lelx*ly1*lely)
...
!     Go through all stat files and read them
      ...
      do ssf = first_file,nfiles

         ! Define name of corresponding stat file
         write(pippo,'(i5.5)') ssf
         inputname1 = 'ZSTAT/stsnaca_wing0.f'//trim(pippo)
...
!      Find inverse (jacmi) of the Jacobian array (jacm1)
          ...
          ! Samuel: ATTENTION! HARDCODED PARAMETERS
          matime_tot = (mtimee - mtimes)
          matime = matime_tot/10  ! Stats were computed every 10 iterations
          mDT = 1.5e-5
          nrec = isteps/10 ! Stats were computed every 10 iterations
...
!     Outpost velocity field, to visualize stat_temp1 and stat_temp2
      write(*,*) 'DEBUG: Writing UV_ ...'
      call outpost(vx,vy,vz,pr,t,'UV_')


!!!!!!!! MA: add u_rms * v_rms for structure identification

      do j=1,m
         stat_temp1(j)=stat_2D(j,5)-stat_2D(j,1)*stat_2D(j,1)
         stat_temp2(j)=stat_2D(j,6)-stat_2D(j,2)*stat_2D(j,2)
      enddo
      call copy(vx,stat_temp1,m)
      call copy(vy,stat_temp2,m)

      write(*,*) 'DEBUG: Writing RMS ...'
      call outpost(vx,vy,vz,pr,t,'RMS')
...
!     Outpost velocity field, to visualize stat_temp1 and stat_temp2
      write(*,*) 'DEBUG: Writing WP_ ...'
      call outpost(vx,vy,vz,pr,t,'WP_')

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! MA: save pressure and boundaries:

      stat_temp3=0;
      do j=1,m
         stat_temp1(j)=stat_2D(j,1)
         stat_temp2(j)=stat_2D(j,2)
         if ((stat_temp1(j)+stat_temp2(j)).eq.0) then
            stat_temp3(j)=1.0
         endif
      enddo

      call copy(vx,stat_temp3,m)

      write(*,*) 'DEBUG: Writing BP_ ...'
      call outpost(vx,vy,vz,pr,t,'BP_')
...
!     Store fields 1 and 2 in stat_temp1 and stat_temp2
      ...
      call copy(vx,stat_temp1,m)
      call copy(vy,stat_temp2,m)
      write(*,*) 'DEBUG: Writing DU_ ...'
      call outpost(vx,vy,vz,pr,t,'DU_')
...
!     Store fields 3 and 4 in stat_temp1 and stat_temp2
      ...
      call copy(vx,stat_temp1,m)
      call copy(vy,stat_temp2,m)
      write(*,*) 'DEBUG: Writing DV_ ...'
      call outpost(vx,vy,vz,pr,t,'DV_')
...
C%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%C
      ...
      write(*,*) ''
      write(*,*) 'Reading interpolating mesh:'
...
!     Name of the file is int_fld
      inputname2 = 'INTERP/int_fld'

      write(*,*) 'DEBUG: Writing interpolation'
...
!     SEM mesh, we proceed to interpolate
      do ifld=1,nstat
         ...
         call findpts_eval(inth_hpts,fieldout,1,
     &        rcode,1,
     &        proc,1,
     &        elid,1,
     &        rst,ndim,npoints,
