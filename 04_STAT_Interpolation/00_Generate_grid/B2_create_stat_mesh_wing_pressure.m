...
disp(["length x_pts", num2str(length(x_pts))])
disp(["length x_pts(mask)", num2str(length(mesh_pressure_side.x_pts))])
...
% ymin_plot = 0;
% ymax_plot = 0.25;
% plot_mask = (mesh_pressure_side.x_pts > xmin_plot) & (mesh_pressure_side.x_pts < xmax_plot) & ...
%             (mesh_pressure_side.y_pts > ymin_plot) & (mesh_pressure_side.y_pts < ymax_plot);

figure
[xu_plot, yu_plot, ~, xl_plot, yl_plot, ~, ~] = naca_prof(m, p, xx, 100, tol);
% h(1) = scatter(mesh_pressure_side.x_pts(plot_mask),mesh_pressure_side.y_pts(plot_mask),'.'); hold on
...
axis equal
hold off
saveas(gcf,'stat_mesh_pressure.png');

%% export mesh

save_inter_stat_mesh('./ZSTAT/',mesh_pressure_side);

%%
int_mesh=mesh_pressure_side;
% int_mesh=mesh_pressure_side;
save('int_stat_mesh_pressure.mat',"int_mesh",'-mat')
