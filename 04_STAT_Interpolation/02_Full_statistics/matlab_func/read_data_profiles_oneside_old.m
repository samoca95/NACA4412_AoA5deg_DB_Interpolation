function [top,bottom] = read_data_profiles_oneside(side,path,binary_file,x_pts,y_pts,alpha,long,xc,yn)
...
for i=1:np
    bottom(i).xa=x_pts(ln*(i-1)+1);
    bottom(i).ya=y_pts(ln*(i-1)+1);
    bottom(i).xc=xc(i);
    bottom(i).alpha=alpha(i);
    bottom(i).yn=yn;
    bottom(i).x=x_pts(ln*(i-1)+1:ln*(i-1)+ln);
    bottom(i).y=y_pts(ln*(i-1)+1:ln*(i-1)+ln);
end
...
for ii=1:nstat+nderiv
    ...
    % Duplicate field for bottom side
    for i=1:np
        Mat(:,i+np)=Mat(:,i);
    end
