function [top,bottom] = read_data_profiles_checkres(path,x_pts,y_pts,alphal,alphau,long,xc,yninput)
...
for i = 1:npts
    for j = 1:nyn
        top(i).taosn(j) = mu*(top(i).dUdy(j)+top(i).dVdx(j))-rhoinf*top(i).uv(j);
        bottom(i).taosn(j) = mu*(bottom(i).dUdy(j)+bottom(i).dVdx(j))-rhoinf*bottom(i).uv(j);
        top(i).taosz(j) = mu*(top(i).dWdx(j))-rhoinf*top(i).uw(j);
        bottom(i).taosz(j) = mu*(bottom(i).dWdx(j))-rhoinf*bottom(i).uw(j);
        top(i).taonz(j) = mu*(top(i).dWdy(j))-rhoinf*top(i).vw(j);
        bottom(i).taonz(j) = mu*(bottom(i).dWdy(j))-rhoinf*bottom(i).vw(j);
        
        top(i).taoss(j) = 2*mu*top(i).dUdx(j)-rhoinf*top(i).uu(j);
        bottom(i).taoss(j) = 2*mu*bottom(i).dUdx(j)-rhoinf*bottom(i).uu(j);
        top(i).taonn(j) = 2*mu*top(i).dVdy(j)-rhoinf*top(i).vv(j);
        bottom(i).taonn(j) = 2*mu*bottom(i).dVdy(j)-rhoinf*bottom(i).vv(j);
        top(i).taozz(j) = -rhoinf*top(i).ww(j);
        bottom(i).taozz(j) = -rhoinf*bottom(i).ww(j);
    end
end

% 6. Mean pressure profiles
for i = 1:npts
    top(i).P = top(i).P-pinf;
    bottom(i).P = bottom(i).P-pinf;
end

% 1. Overall mean-force coefficients
for i = 1:npts
    Pt(i) = top(i).P(1)+pinf;
    Pb(i) = bottom(i).P(1)+pinf;
    tau_nn_t(i) = top(i).taonn(1);
    tau_nn_b(i) = bottom(i).taonn(1);
    tau_sn_t(i) = top(i).taosn(1);
    tau_sn_b(i) = bottom(i).taosn(1);
    tau_ss_t(i) = top(i).taoss(1);
    tau_ss_b(i) = bottom(i).taoss(1);
