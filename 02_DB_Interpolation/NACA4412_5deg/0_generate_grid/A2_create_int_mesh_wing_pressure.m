...
utz = 0.0445;        % Friction velocity at x/c=0.4            
...
mesh_pressure_side.yn    = yn;    % Vector normall to the wall
mesh_pressure_side.xs    = xsl;   % Vector tangent to the wall     
mesh_pressure_side.xc    = xc;    % Vector of chordwise coordinates
mesh_pressure_side.z     = zvec;  % Vector of spanwise coordinates

%%
% Info about the mesh
Nx = length(mesh_pressure_side.xc); 
Ny = length(mesh_pressure_side.yn);
Nz = length(mesh_pressure_side.z);
[~, ix04]   = min(abs(mesh_pressure_side.xc - 0.4));
dxc_04_plus = (mesh_pressure_side.xc(ix04+1) - mesh_pressure_side.xc(ix04)) * utz / nu;
dxc_plus = diff(mesh_pressure_side.xc) * utz / nu;
dxs_plus = diff(mesh_pressure_side.xs) * utz / nu;
dxs_04_plus = dxs_plus(ix04);
dyn_plus = diff(mesh_pressure_side.yn) * utz / nu;
dz_plus  = (mesh_pressure_side.z(2) - mesh_pressure_side.z(1)) * utz / nu;
...
disp(['x/c range: ', num2str(min(mesh_pressure_side.xc)), ' - ', num2str(max(mesh_pressure_side.xc))]);
disp(['y/c range: ', num2str(min(mesh_pressure_side.yn)), ' - ', num2str(max(mesh_pressure_side.yn))]);
disp(['z/c range: ', num2str(min(mesh_pressure_side.z)), ' - ', num2str(max(mesh_pressure_side.z))]);
disp(['xmin=', num2str(min(mesh_pressure_side.x_pts)), ' xmax=', num2str(max(mesh_pressure_side.x_pts))]);
disp(['ymin=', num2str(min(mesh_pressure_side.y_pts)), ' ymax=', num2str(max(mesh_pressure_side.y_pts))]);
disp(['zmin=', num2str(min(mesh_pressure_side.z_pts)), ' zmax=', num2str(max(mesh_pressure_side.z_pts))]);
...
fprintf(logID,'Dz+=%f\n',dz_plus(1));
fprintf(logID,'\n');
fprintf(logID,'Ntotal=%d\n',length(mesh_pressure_side.x_pts));
fprintf(logID,'Nx=%d Ny=%d Nz=%d\n',Nx,Ny,Nz);
fprintf(logID,'\n');
fprintf(logID,'x/c range: %f - %f\n', min(mesh_pressure_side.xc), max(mesh_pressure_side.xc));
fprintf(logID,'y/c range: %f - %f\n', min(mesh_pressure_side.yn), max(mesh_pressure_side.yn));
fprintf(logID,'z/c range: %f - %f\n', min(mesh_pressure_side.z), max(mesh_pressure_side.z));
fprintf(logID,'xmin=%f xmax=%f\n', min(mesh_pressure_side.x_pts), max(mesh_pressure_side.x_pts));
fprintf(logID,'ymin=%f ymax=%f\n', min(mesh_pressure_side.y_pts), max(mesh_pressure_side.y_pts));
fprintf(logID,'zmin=%f zmax=%f\n', min(mesh_pressure_side.z_pts), max(mesh_pressure_side.z_pts));
fprintf(logID,'\n');
...
h(1) = scatter(mesh_pressure_side.x_pts,mesh_pressure_side.y_pts,'.'); hold on
